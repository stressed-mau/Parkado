...
export default function DashboardParqueo() {
  const parqueoId = 1; // estático por ahora
  ...
  // ruta local del archivo que subiste (la dejo tal cual en el historial)
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!mounted) return;
        // asumimos que el backend regresa el objeto con las keys documentadas
  ...
  // Si el backend no devuelve ciertos campos, usamos fallback 0/N-D
  ...
  const tasa = dashboard?.tasaOcupacion ?? `${plazasTotales ? ((plazasOcupadas / plazasTotales) * 100).toFixed(1) + "%" : "0%"}`;

  return (
    <ScrollView className="flex-1 px-4 py-6" style={{ backgroundColor: colores.crema }}>
      {/* HEADER: logo + título */}
      <View className="items-center mb-6">
        {/* si tienes el componente Logo, se muestra; mantengo el logoLocalUri para fallback */}
        <View className="flex-row items-center">
          {/* componente Logo si existe */}
          <Logo width={72} height={72} />
          {/* y también mostramos la imagen local (por si quieres ver el archivo subido) */}
          <Image
      ...
      <View className="mb-4 rounded-2xl p-4" style={{ backgroundColor: "#ffffff", shadowColor: "#00000020", shadowOpacity: 0.12, shadowRadius: 8 }}>
        ...
        <View className="flex-row justify-between">
          <View className="flex-1 mr-2 rounded-xl p-3" style={{ backgroundColor: "#f7faf9" }}>
            ...
            <Text className="text-xl font-bold mt-2">${Number(ingresos).toLocaleString()}</Text>
          </View>
        ...
        <View className="flex-row justify-between mt-3">
          <View className="flex-1 mr-2 rounded-xl p-3" style={{ backgroundColor: "#f7faf9" }}>
            ...
            <Text className="text-xl font-bold mt-2">{rating !== null ? rating.toFixed(1) : "N/D"}</Text>
          </View>
      ...
      {/* FOOTNOTE: JSON crudo (útil para debug) */}
